...
for anisotropy in [0, 0.2, 0.4, 0.6]:
    i = 0
    print(Path(bpy.path.abspath("//bear/" + str(anisotropy) + f"/input-{i:02}.png")))
    ...
    for i in range(len(camData["CamPos"])):
        ...
        bpy.data.materials["Brushed Metal"].node_tree.nodes["Anisotropic BSDF"].inputs[2].default_value = anisotropy
        bpy.context.scene.render.filepath = str(Path(bpy.path.abspath("//bear/" + str(anisotropy) + f"/input-{i:02}.png")))
        ...
        dmap = np.flipud(dmap)
        np.save(str(Path(bpy.path.abspath("//bear/" + str(anisotropy) + f"/depth-{i:02}"))), dmap)
