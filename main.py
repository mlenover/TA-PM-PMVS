...
import csv
import os
from generate_images_blender import *
...
def main(exp_name, input_file, max_dist, min_dist, kernel_connectivity, lambda_c, no_outter_iterations, no_inner_iterations, no_qpm_iterations, no_patchmatch_iterations, theta_d_dev, len_dev, outter_attenuation, inner_attenuation, no_candidates, lambda_n, lambda_z_init, lambda_z_increase_rate_outter, lambda_z_increase_rate_inner, lambda_l, filename, shape, anisotropy, epsilon=1e-10):
    ...
    depth_err = np.zeros(no_outter_iterations)
    depth_acc = np.zeros(no_outter_iterations)
    ...
    with tqdm(total=no_outter_iterations*no_inner_iterations) as pbar:
        for _i_ in range(no_outter_iterations):
            ...
            plot_brdf(log_brdf_func, xs, ys, filename+f'/{exp_name}-{_i_:02}-f.png')
            ######################################
            ...
            if _i_ == 0:
                ...
                cv2.imwrite(filename+f'/{exp_name}-{_i_:02}-shape.png', (l_img*255).astype(np.uint8)[...,::-1].reshape([height, width, 3]))
                cv2.imwrite(filename+f'/{exp_name}-{_i_:02}-normal.png', (n_img*255).astype(np.uint8)[...,::-1].reshape([height, width, 3]))
                cv2.imwrite(filename+f'/{exp_name}-{_i_:02}-depth.png', (d_img*255).astype(np.uint8).reshape([height, width]))
            ##########################################################################3
            ...
            est_depth = d_img.copy()
            depth_err, depth_acc = calc_depth_error(est_depth, _i_, depth_err, depth_acc, filename, shape, anisotropy)

            np.reshape(d_img, [400, 640])
            np.save(filename+f'/est-depth-{_i_:02}', est_depth)
            ...
            #act_depth = np.load('results/depth-00.npy', allow_pickle=True)
            # est_depth = np.load('results/est-depth-00.npy', allow_pickle=True)
            #est_depth = np.reshape(est_depth, [400, 640])

            #est_depth = np.where(est_depth == np.inf, np.nan, est_depth)
            #av_depth = np.abs(act_depth - est_depth)
            #av_depth = np.where(av_depth == np.inf, np.nan, av_depth)
            #av_err = np.nanmean(np.ndarray.flatten(av_depth))



    # np.save('Raw Training Data/Output Depth Map/pmvs_depth.npy', d_raw)
...
with open('Test Scenarios.csv', newline='') as f:
    reader = csv.reader(f)
    test_params = list(reader)[1:]

dirname = os.path.dirname(__file__)

for i, test_line in enumerate(test_params):
    filename = f'results-{i:02}'

    if not os.path.isdir(f'./results-{i:02}'):
        os.mkdir(os.path.join(dirname, filename))

    packInputFile(test_line[0], test_line[1], test_line[2], test_line[3], filename)
